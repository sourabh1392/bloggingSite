...
const getBlog=async function(req,res){
    try{
        ...
        res.status(200).send({status:true,message:"BlogList",data:blogdata })
    }
    catch(err){
        res.status(500).send(err.message)
    }
...
const getPerticularBlog=async function(req,res){
    try{
        ...
        if(!getdata) return res.status(400).send("No such blog found")
        res.status(200).send({status:true,message:"Filtered Blog",data:getdata})
    }
    catch(err){
        res.status(500).send(err.message)
    }
